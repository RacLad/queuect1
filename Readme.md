...
1. **Job Enqueue** → User adds a new command.  
   Stored in `jobs.json` with state `"pending"`.

2. **Worker Loop** → Continuously polls `jobs.json` for pending jobs.
...
- This project demonstrates a foundational understanding of **asynchronous processing**, **persistent state management**, and **fault-tolerant system design**.  
- It is designed to run entirely on the local file system — no external database or queue manager required.
